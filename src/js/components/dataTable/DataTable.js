...
function drawText(pressArticles, settingEllipsis, isMobile) {
  pressArticles.forEach((article) => {
    ...
    if (isMobile) {
      title.textContent = ellipsisText(
        title.textContent,
        settingEllipsis.mobile.title
      );
      desc.textContent = ellipsisText(
        desc.textContent,
        settingEllipsis.mobile.desc
      );
    } else {
      restoreText(title, desc);
      title.textContent = ellipsisText(
        title.textContent,
        settingEllipsis.desktop.title
      );
      desc.textContent = ellipsisText(
        desc.textContent,
        settingEllipsis.desktop.desc
      );
    }
  });
}
...
class DataTable extends Component {
  ...
  _bindEvents() {
    ...
    // 검색 시 처리할 동작
    this.searchForm.onSearch((category, keyword) =>
      this.renderSearchResult(category, keyword)
    );
      ...
      switch (targetTagName) {
        case 'button':
          ...
          if (!attr(target, 'disabled')) {
            ...
            currentPage = hasClass(target, 'button-prev')
              ? previousPage
              : nextPage;
          }
          break;
  ...
  searchKeywordItems(category, keyword) {
    ...
    return data.filter((item) =>
      item[category].match(new RegExp(keyword, 'ig'))
    );
  }
  ...
  removeRendered() {
    ...
    const oldPressTableContailerEl = getNode(
